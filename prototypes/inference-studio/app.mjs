...
function renderInspector(rows) {
  ...
  <section class="inspector-section"><h3>Tokens & provenance</h3>${kv([
      ...
      ["Exact output flag", r.output_token_exact ? "true" : "false"],
  ...
  <section class="inspector-section"><h3>Chunk arrival gaps <span class="inspector-note">n=${gaps.length}</span></h3>${gaps.length ? `${kv([["p50 / max", `${fmt(percentile(gaps, 50))} / ${fmt(Math.max(...gaps))} ms`]])}<div class="gap-bars" aria-label="Chunk interarrival gaps in recorded order">${gaps.map((v, i) => `<i style="height:${Math.max(2, (v / gapMax) * 38)}px" title="Gap ${i + 1}: ${v} ms"></i>`).join("")}</div><button class="text-button" id="toggle-gaps">${state.gaps ? "Hide" : "Show"} all ${gaps.length} gap values</button>${state.gaps ? `<div class="detail-gap">${gaps.map((v, i) => `<span>${i + 1}: ${fmt(v, 3)}</span>`).join("")}</div>` : ""}` : '<p class="inspector-note">No chunk intervals were recorded.</p>'}<p class="inspector-note">A chunk may contain multiple tokens. These gaps are not inter-token latency.</p></section>
